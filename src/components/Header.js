...
import { toggleAiSearchView } from "../utils/aiSearchSlice"; // Updated import
import { changeLanguage } from "../utils/configSlice";

const Header = () => {
  const navigate = useNavigate();
  const user = useSelector((store) => store.user);
  const aiSearch = useSelector((store) => store.aiSearch); // Updated selector
  const dispatch = useDispatch();

  const isAiSearch = aiSearch.showAiSearch; // Updated variable name
  ...
  const handleAiSearchView = () => {
    // Updated function name
    dispatch(toggleAiSearchView()); // Updated action
  };
